...
def _cache_get(namespace, key):
    conn = None
    try:
        conn = _cache_connection(); row = conn.execute("SELECT value FROM scrape_cache WHERE namespace=? AND cache_key=?",(namespace, key),).fetchone()
...
def _cache_set(namespace, key, value):
    conn = None
    try:
        encoded = json.dumps(value); conn = _cache_connection()
        with conn:
            conn.execute("INSERT OR REPLACE INTO scrape_cache(namespace, cache_key, value, created) VALUES(?, ?, ?, ?)",(namespace, key, encoded, time.time()),)
...
def _serper_page(query, page, page_size):
    ...
    response = requests.post(SERPER_URL,headers={**HEADERS, "X-API-KEY": api_key, "Content-Type": "application/json"},json={"q": query, "num": int(page_size), "page": int(page)},timeout=SERPER_RESULT_TIMEOUT,); response.raise_for_status(); payload = response.json(); organic = payload.get("organic", []) or []; results = []
    for item in organic:
        url = _clean_url(item.get("link", "")); title = clean_text(item.get("title", "")); snippet = clean_text(item.get("snippet", ""))
...
def _wikipedia_results(query, limit):
    ...
    if cached is not None: return cached
    response = requests.get(WIKIPEDIA_OPENSEARCH_URL,params={"action": "opensearch","search": query,"limit": min(max(1, int(limit)), 10), "namespace": 0,"format": "json"},headers=HEADERS,timeout=SERPER_RESULT_TIMEOUT); response.raise_for_status(); payload = response.json(); titles = payload[1] if len(payload) > 1 else []
...
def search_results(query, limit=SCRAPE_DEFAULT_LIMIT, offset=0):
    ...
    if not query: return []
    limit = max(0, int(limit)); offset = max(0, int(offset)); provider_limit = max(1, limit + offset); results = []; errors = []
    for provider in (_serper_results, _wikipedia_results):
        try: results.extend(provider(query, provider_limit))
        except Exception as exc: errors.append(str(exc))
        if len(results) >= provider_limit: break
    seen = set(); unique = []
    for item in results:
...
def blocks_for_query(query, limit=SCRAPE_DEFAULT_LIMIT, offset=0, prefer_snippets=False):
    ...
    for result in search_results(query, limit=limit, offset=offset):
        url = _clean_url(result.get("url", "")); title = clean_text(result.get("title", ""))
        if prefer_snippets:
            ...
            if block and len(block["content"]) >= MIN_EXTRACTABLE_BLOCK_CHARS:
                ...
                continue
        try: text = _fetch_url_text(url)
        except Exception: text = ""
        if len(text) >= MIN_EXTRACTABLE_BLOCK_CHARS:
            block = {"content": text[:MAX_CHARS_PER_BLOCK],"tag": _result_tag(title, url),"url": url,}
        else: block = _snippet_block(result)
        if block:
            fingerprint = _block_fingerprint(block)
            if fingerprint in seen_blocks: continue
            seen_blocks.add(fingerprint); blocks.append(block)
        if len(blocks) >= limit: break
    return blocks
...
def scrape_worker_loop(scrape_queue, extract_queue, stop_event, sync_counter):
    while not stop_event.is_set():
        try: task = scrape_queue.get(timeout=0.2)
        except Empty: continue
        query = clean_text(task.get("query", "") if isinstance(task, dict) else task); limit = task.get("limit", SCRAPE_DEFAULT_LIMIT) if isinstance(task, dict) else SCRAPE_DEFAULT_LIMIT; offset = task.get("offset", 0) if isinstance(task, dict) else 0; research_pass = task.get("research_pass", "primary") if isinstance(task, dict) else "primary"
        try:
            blocks = blocks_for_query(query, limit=limit, offset=offset, prefer_snippets=research_pass == "refine_existing"); extract_queue.put({"query": query, "blocks": blocks, "connections": [],"research_pass": research_pass,"limit": int(limit),"offset": int(offset),})
        except Exception as exc:
            extract_queue.put({"query": query, "blocks": [],"connections": [],"error": str(exc),"research_pass": research_pass,"limit": int(limit),"offset": int(offset),})
        finally:
            with sync_counter.get_lock(): sync_counter.value = max(0, sync_counter.value - 1)
            time.sleep(0.05)
