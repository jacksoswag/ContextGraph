...
from d_noise_cleanup import is_clause_like_agent_text, is_usable_agent_text
from d_word_info_map import str_to_vector
...
def is_useful_thought_agent(agent):
    ...
    if not tokens:
        return False
    if len(tokens) < 2 and len(name) < 3:
        return False
    return True
...
def lexical_target_score(_brain, agent_name, target_text, require_distinctive=False):
    ...
    distinctive_tokens = distinctive_target_tokens(target_tokens_set) if require_distinctive else set()
...
def expanded_queries_for_side(brain, side):
    side = str(side or "").strip().lower()
    attr = "target_a_queries" if side == "a" else "target_b_queries"
    return list(getattr(brain, attr, []) or [])


def completion_queries_for_side(brain, side):
    target = target_text_for_side(brain, side)
    queries = normalize_seed_queries([target])
    if queries:
        return queries
    queries = normalize_seed_queries(expanded_queries_for_side(brain, side))
    if queries:
        return queries
    fallback = " ".join(str(target or "").strip().split())
    return [fallback] if fallback else []


def route_directions():
    return (
        {"id": "a_to_b", "start_side": "a", "goal_side": "b", "start_label": "A", "goal_label": "B"},
        {"id": "b_to_a", "start_side": "b", "goal_side": "a", "start_label": "B", "goal_label": "A"},
    )
...
def _agent_endpoint_context(agent, conn):
    ...
    target_agent = getattr(conn, "target", None)
...
def agent_grounding_texts(agent, connector_limit=18):
    ...
    seen = set()
    name = str(getattr(agent, "ASU", "") or "").strip()
    _append_grounding_text(texts, seen, name)
    connectors = sorted(
        list(getattr(agent, "connectors", []) or []),
        key=lambda conn: float(getattr(conn, "utility", 0.0) or 0.0),
        reverse=True,
    )
    for conn in connectors[:max(0, int(connector_limit))]:
        endpoint, endpoint_specifics = _agent_endpoint_context(agent, conn)
        if endpoint is not None:
            modifier_text = " ".join(endpoint.modifier_value()).strip()
            _append_grounding_text(texts, seen, modifier_text)
        ...
        evidence = str(getattr(conn, "evidence_text", "") or "").strip()
        _append_grounding_text(texts, seen, evidence)
    return texts
...
def _agent_lexical_target_score(brain, agent, target_text, require_distinctive=False):
    return max(
        (
            lexical_target_score(
                brain,
                text,
                target_text,
                require_distinctive=require_distinctive,
            )
            for text in agent_grounding_texts(agent)
        ),
        default=0.0,
    )
...
def _rank_agents_by_name_similarity(brain, queries, require_distinctive=False):
    ...
    try:
        query_vecs = [
            np.asarray(str_to_vector(query), dtype=np.float32)
            for query in queries
            if query
        ]
    except Exception:
    ...
    for agent in brain.agents.values():
        ...
        name_lexical_score = _agent_name_target_score(
            brain,
            agent,
            queries,
        ...
        if query_vecs:
            try:
                agent_vec = np.asarray(str_to_vector(name), dtype=np.float32)
                vector_score = max(float(np.dot(agent_vec, query_vec)) for query_vec in query_vecs)
            except Exception:
                vector_score = 0.0
        own_grounding_score = max(float(name_lexical_score), float(vector_score))
        overall_score = max(own_grounding_score, 0.65 * float(lexical_score))
        overall_score *= anchor_quality
        ...
        ranked.append(
            (
                ...
                float(lexical_score),
                float(vector_score),
                float(name_lexical_score),
                len(getattr(agent, "connectors", ()) or ()),
                name,
...
def _rank_anchor_candidates(brain, side, limit):
    ...
    if not queries:
        return []
    opposite_side = "b" if str(side or "").strip().lower() == "a" else "a"
    opposite_target = target_text_for_side(brain, opposite_side)
    ranked = _rank_agents_by_name_similarity(brain, queries, require_distinctive=True)
    candidates = []
    for score, lexical_score, vector_score, name_score, degree, name, anchor_quality, agent in ranked:
        ...
        if float(score) < float(PATH_TARGET_MATCH_THRESHOLD):
            continue
        if float(name_score) < float(PATH_TARGET_MATCH_THRESHOLD):
            continue
        if opposite_target:
            opposite_name_score = lexical_target_score(
                brain,
                name,
                opposite_target,
        ...
        best_query = max(
            queries,
            key=lambda query: _agent_lexical_target_score(brain, agent, query),
        )
        candidates.append(
            {
                "agent": agent,
...
def _connection_specific_count(brain, key):
    ...
    return sum(
        len(list(specifics.get(field, []) or []))
...
def _route_metrics(
    ...
    empty = {
        ...
        "source_count": 0,
        "specific_count": 0,
        "specific_edge_count": 0,
        "score": 0.0,
    }
    start_index = getattr(start_agent, "index", None)
    goal_index = getattr(goal_agent, "index", None)
    if start_index is None or goal_index is None:
        return empty
    start_index = int(start_index)
    goal_index = int(goal_index)

    forward_reach = forward_reach or {}
    backward_reach = backward_reach or {}
    from_start = forward_reach.get(start_index)
    ...
    for source, edges in adjacency.items():
        ...
        for target, key in edges:
            if int(target) not in to_goal:
                continue
            route_edges.add(key)
            route_nodes.add(int(source))
            route_nodes.add(int(target))
            display = display_source(getattr(brain, "connection_sources", {}).get(key, ""))
            if display and display != "unknown":
                sources.add(display.rstrip("/").lower())
            edge_specific_count = _connection_specific_count(brain, key)
            specific_count += edge_specific_count
    ...
    score = (
        min(edge_count, 96)
        + (2.5 * min(source_count, 16))
        + (1.25 * min(specific_edge_count, 48))
        + (0.2 * min(specific_count, 160))
    )
    return {
...
def _pair_diagnostic(pair, selected=False):
    a_metrics = pair.get("a_to_b_metrics", {}) or {}
    b_metrics = pair.get("b_to_a_metrics", {}) or {}
    return {
        "selected": bool(selected),
        "a": pair["a"].get("name", ""),
        "b": pair["b"].get("name", ""),
        "a_score": round(float(pair["a"].get("score", 0.0) or 0.0), 3),
        "b_score": round(float(pair["b"].get("score", 0.0) or 0.0), 3),
        "a_to_b_edges": int(a_metrics.get("edge_count", 0) or 0),
        "b_to_a_edges": int(b_metrics.get("edge_count", 0) or 0),
        "sources": int(a_metrics.get("source_count", 0) or 0) + int(b_metrics.get("source_count", 0) or 0),
        "specific_edges": int(a_metrics.get("specific_edge_count", 0) or 0) + int(b_metrics.get("specific_edge_count", 0) or 0),
        "rank": tuple(round(float(value), 3) for value in pair.get("rank", ())),
    }


def _connected_anchor_pairs(brain, candidate_limit, pair_limit=None):
    a_candidates = _rank_anchor_candidates(brain, "a", candidate_limit)
    b_candidates = _rank_anchor_candidates(brain, "b", candidate_limit)
    adjacency = _directed_adjacency_from_connections(brain)
    reverse = _reverse_adjacency(adjacency)
    candidate_indices = {
        int(item["agent"].index)
        for item in list(a_candidates or []) + list(b_candidates or [])
        if getattr(item.get("agent"), "index", None) is not None
    }
    forward_reach = {
        index: _reachable_indices(adjacency, [index])
        for index in candidate_indices
    }
    backward_reach = {
        index: _reachable_indices(reverse, [index])
        for index in candidate_indices
    }
    pairs = []
    ...
    for a_item in a_candidates:
        ...
        for b_item in b_candidates:
            ...
            a_to_b = a_to_b_metrics["edge_count"] > 0
            b_to_a = b_to_a_metrics["edge_count"] > 0
            if not a_to_b and not b_to_a:
                continue
            route_score = float(a_to_b_metrics["score"]) + float(b_to_a_metrics["score"])
            anchor_floor = min(float(a_item["score"]), float(b_item["score"]))
            anchor_sum = float(a_item["score"]) + float(b_item["score"])
            source_count = int(a_to_b_metrics["source_count"]) + int(b_to_a_metrics["source_count"])
            specific_edges = int(a_to_b_metrics["specific_edge_count"]) + int(b_to_a_metrics["specific_edge_count"])
            rank = (
                route_score * max(0.25, anchor_floor),
                route_score,
                source_count,
                specific_edges,
                1 if a_to_b and b_to_a else 0,
                anchor_floor,
                anchor_sum,
                a_item["name_score"] + b_item["name_score"],
                a_item["degree"] + b_item["degree"],
            )
            pairs.append({
                "a": a_item,
                "b": b_item,
                "a_to_b": a_to_b,
                "b_to_a": b_to_a,
                "a_to_b_metrics": a_to_b_metrics,
                "b_to_a_metrics": b_to_a_metrics,
                "rank": rank,
            })

    if not pairs:
        brain.anchor_pair_diagnostics = []
        return []

    pairs.sort(key=lambda item: item["rank"], reverse=True)
    pair_limit = max(1, int(pair_limit or ACTIVE_ANCHOR_PAIR_LIMIT))
    selected = pairs[:pair_limit]
    selected_keys = {
        (int(pair["a"]["agent"].index), int(pair["b"]["agent"].index))
        for pair in selected
    }
    diagnostic_pairs = selected + [
        pair for pair in pairs[pair_limit:pair_limit + 8]
    ]
    seen_diag = set()
    brain.anchor_pair_diagnostics = []
    for pair in diagnostic_pairs:
        ...
        if key in seen_diag:
            continue
        seen_diag.add(key)
        brain.anchor_pair_diagnostics.append(_pair_diagnostic(pair, selected=key in selected_keys))
    return selected
...
def _route_spawn_counts(active_count, total_thoughts):
    ...
    total = max(active_count, int(total_thoughts or active_count))
    base = total // active_count
    remainder = total % active_count
    return [base + (1 if idx < remainder else 0) for idx in range(active_count)]


def _route_seed_spec(anchor_item, spawn_count):
    return {
        "agent": anchor_item["agent"],
        "query": anchor_item.get("query", ""),
        "score": float(anchor_item.get("score", 0.0) or 0.0),
        "spawn_count": max(1, int(spawn_count or 1)),
    }
...
def _with_redistributed_route_spawns(routes, total_thoughts):
    ...
    spawn_counts = _route_spawn_counts(len(active_refs), total_thoughts)
    spawn_by_ref = dict(zip(active_refs, spawn_counts))
    ...
    for idx, route in enumerate(list(routes or [])):
        ...
        next_route["seed_specs"] = next_specs
        next_route["seed_agents"] = [item["agent"] for item in next_specs if item.get("agent") is not None]
        next_route["goal_agents"] = list(route.get("goal_agents", []) or [])
        redistributed.append(next_route)
...
def select_thought_routes(brain, seed_limit=None, goal_limit=None, total_thoughts=None):
    candidate_limit = max(
        1,
        int(seed_limit or TARGET_SEED_LIMIT),
        int(goal_limit or GOAL_AGENT_LIMIT),
    )
    pairs = _connected_anchor_pairs(
        brain,
        candidate_limit,
        pair_limit=ACTIVE_ANCHOR_PAIR_LIMIT,
    )
    routes = []
    for route_index, direction in enumerate(route_directions()):
        start_side = direction["start_side"]
        goal_side = direction["goal_side"]
        seed_queries = completion_queries_for_side(brain, start_side)
        goal_queries = completion_queries_for_side(brain, goal_side)
        seed_specs = []
        goal_agents = []
        for pair in pairs:
            if direction["id"] == "a_to_b" and pair["a_to_b"]:
                seed_specs.append(_route_seed_spec(pair["a"], 1))
                goal_agents.append(pair["b"]["agent"])
            elif direction["id"] == "b_to_a" and pair["b_to_a"]:
                seed_specs.append(_route_seed_spec(pair["b"], 1))
                goal_agents.append(pair["a"]["agent"])
        seed_specs = [_route_seed_spec(item, 1) for item in _unique_anchor_items(seed_specs)]
        goal_agents = _unique_goal_agents(goal_agents)
        routes.append(
            {
                **direction,
                "index": route_index,
                "start_target": target_text_for_side(brain, start_side),
                "goal_target": target_text_for_side(brain, goal_side),
                "seed_queries": seed_queries,
                "goal_queries": goal_queries,
                "seed_specs": seed_specs,
                "seed_agents": [item["agent"] for item in seed_specs],
                "goal_agents": list(goal_agents),
                "anchor_locked": True,
            }
        )
...
def apply_thought_routes(brain, routes, create_python_thoughts=True):
    brain.target_thought_routes = [dict(route) for route in list(routes or [])]
    seed_specs = []
    goal_agents = []
    for route in brain.target_thought_routes:
        route["seed_specs"] = list(route.get("seed_specs", []) or [])
        route["seed_agents"] = [item["agent"] for item in route["seed_specs"] if item.get("agent") is not None]
        route["goal_agents"] = list(route.get("goal_agents", []) or [])
        seed_specs.extend(route["seed_specs"])
        goal_agents.extend(route["goal_agents"])

    brain.target_seed_specs = seed_specs
    brain.target_seed_agents = _unique_agents(item["agent"] for item in seed_specs if item.get("agent") is not None)
    brain.target_goal_agents = _unique_agents(goal_agents)
    brain.thoughts = []

    if not create_python_thoughts:
        return

    for route in brain.target_thought_routes:
        for item in route.get("seed_specs", []):
            agent = item.get("agent")
            if agent is None:
                continue
            for _ in range(max(1, int(item.get("spawn_count", 1)))):
                thought = Thought(
                    agent,
                    seed_query=item.get("query", ""),
                    start_target=route.get("start_target", ""),
                    target_b=route.get("goal_target", ""),
                    success_queries=route.get("goal_queries", []),
                    goal_agents=route.get("goal_agents", []),
                )
                thought.route_id = route.get("id", "")
                thought.route_start_label = route.get("start_label", "")
                thought.route_goal_label = route.get("goal_label", "")
                brain.thoughts.append(thought)


def refresh_target_seed_agents(brain, limit=None, total_thoughts=None, create_python_thoughts=True):
    clear_thought_caches()
    existing_routes = list(getattr(brain, "target_thought_routes", []) or [])
    if existing_routes and all(route.get("anchor_locked") for route in existing_routes):
        ...
        routes = select_thought_routes(
            brain,
            seed_limit=limit,
            goal_limit=GOAL_AGENT_LIMIT,
            total_thoughts=total_thoughts,
        )
    apply_thought_routes(brain, routes, create_python_thoughts=create_python_thoughts)
    if brain.target_seed_agents:
        thought_count = sum(
            int(item.get("spawn_count", 1))
            for route in brain.target_thought_routes
            for item in route.get("seed_specs", [])
        )
        print(
            f"[THOUGHT] Seeded {thought_count} thought agents across "
        ...
        for route in brain.target_thought_routes:
            seeds = route.get("seed_agents", [])
            goals = route.get("goal_agents", [])
            seed_preview = ", ".join(agent.ASU for agent in seeds[:5]) or "(none)"
            goal_preview = ", ".join(agent.ASU for agent in goals[:5]) or "(none)"
            print(
                f"[THOUGHT] Route {route.get('start_label')}->{route.get('goal_label')} "
                f"seeds: {seed_preview}"
            )
            print(
                f"[THOUGHT] Route {route.get('start_label')}->{route.get('goal_label')} "
                f"goals: {goal_preview}"
            )
    else:
...
def _payload_passes_success_threshold(payload):
    if not isinstance(payload, dict):
        return False
    if payload.get("endpoint_reached"):
        return True
    return not (
...
def successful_thoughts(brain):
    strict_successes = [
        ...
        and _payload_passes_success_threshold(getattr(thought, "success_payload", None))
    ]
    if strict_successes:
    ...
    candidates = []
    for thought in brain.thoughts:
        if getattr(thought, "termination_reason", "") != "endpoint":
            continue
        ...
        try:
            ...
            continue
        if not _payload_passes_success_threshold(payload):
            continue
...
def thought_swarm_stats(brain):
    ...
    expected = int(getattr(brain, "thought_expected_count", 0) or 0)
    total = len(brain.thoughts) or expected
    alive = expected if process_alive and not brain.thoughts else sum(
        1 for thought in brain.thoughts if getattr(thought, "alive", False)
    )
...
def _thought_binary_path():
    return os.path.join(os.getcwd(), "thought_engine")
...
def _neighbor_indices_for_agent(agent):
    ...
    for conn in list(getattr(agent, "connectors", []) or []):
        source_agent = getattr(conn, "source_agent", None)
        target_agent = getattr(conn, "target", None)
        source_index = getattr(source_agent, "index", None)
        target_index = getattr(target_agent, "index", None)
...
def _reachable_goal_agents(brain, start_agent, goal_agents):
    ...
    goal_by_index = {
        int(goal.index): goal
        for goal in list(goal_agents or [])
        if getattr(goal, "index", None) is not None
    }
    if not goal_by_index:
        return []
...
def _assigned_cpp_seed_lines(brain):
    lines = []
    routes = list(getattr(brain, "target_thought_routes", []) or [])
    if not routes:
        routes = [
            {
                "index": 0,
                "seed_specs": list(getattr(brain, "target_seed_specs", []) or []),
                "goal_agents": list(getattr(brain, "target_goal_agents", []) or []),
            }
        ]
    ...
    for route in routes:
        ...
        if not goal_agents:
            continue
        route_index = int(route.get("index", 0) or 0)
        thought_index = 0
        for item in list(route.get("seed_specs", []) or []):
            ...
            for _ in range(spawn_count):
                goal = reachable_goals[thought_index % len(reachable_goals)]
                lines.append(f"seed {int(agent.index)} 1 {int(goal.index)} {route_index}")
                thought_index += 1
    return lines
...
def _write_cpp_thought_input(brain, path):
    lines = [
        f"max_hops {int(MAX_THOUGHT_HOPS)}",
    ]
    lines.extend(_assigned_cpp_seed_lines(brain))
...
def _connector_for_record_index(brain, connection_index, offset_keys):
    ...
    try:
        (
            ...
            predicate_tense,
            predicate_truth,
        ) = struct.unpack_from("<iiifiiiiii", brain.shm_connections.buf, offset)
    except struct.error:
        return None

    subject_sp = ConnectionEndpoint(subject_quant, subject_tense, subject_truth, s_idx)
    predicate_sp = ConnectionEndpoint(predicate_quant, predicate_tense, predicate_truth, o_idx)
    key = offset_keys.get(offset) or p_connection_graph._exact_connection_signature(
        s_idx,
        o_idx,
        relation_id,
        subject_sp,
    ...
    state = brain.connection_states.get(key, {})
    specifics = brain.connection_specifics.get(key, {})
    previous_agent_ids = brain.connection_previous_agents.get(key, ())

    return Connector(
        brain.shm_connections.buf,
        offset,
        brain.agents_by_idx,
        ...
        source=brain.connection_sources.get(key, "unknown"),
        evidence_text=brain.connection_texts.get(key, ""),
...
def _remember_specific_context(thought, details):
    for detail in list(details or []):
        ...
        suffix = f" [{display_source(source)}]" if thought._source_is_citable(source) else ""
        thought._remember_note(f"{detail.get('text', '')}{suffix}")


def _append_cpp_step(thought, brain, step, offset_keys):
    conn = _connector_for_record_index(brain, step["connection_index"], offset_keys)
    from_agent = brain.agents_by_idx.get(step["from"])
    to_agent = brain.agents_by_idx.get(step["to"])
    if conn is None or from_agent is None or to_agent is None:
        return False

    thought.current_asu = from_agent
    specific_details = thought._specific_context_details(conn, limit=MAX_CONTEXT_SAMPLES)
    _remember_specific_context(thought, specific_details)

    if conn.evidence_text:
        thought._remember_note(
            thought._detail_from_record(
                {"text": conn.evidence_text, "source": conn.source},
                utility=getattr(conn, "utility", 0.0),
            )
        )
    ...
    subject_agent = conn.source_agent
    predicate_agent = conn.target
    subject_text = _endpoint_text(subject_agent, conn.subject) if subject_agent else ""
    predicate_text = _endpoint_text(predicate_agent, conn.predicate) if predicate_agent else ""
    thought.last_asu_idx = from_agent.index
    thought.current_asu = to_agent
    thought.history.append(
        {
            ...
            "traversal_reversed": bool(step["reversed"]),
            "utility": round(float(getattr(conn, "utility", 0.0) or 0.0), 4),
            "score": round(float(thought.score), 2),
            "details": list(thought.collected_notes[-3:]),
        }
    )
...
def _fallback_success_payload(thought):
    clauses = []
    records = []
    seen = set()
    for step in thought.history[1:]:
        clause = thought._step_clause(step)
        if not clause:
            continue
        source = str(step.get("source", "")).strip()
        key = (clause.strip().lower(), display_source(source).rstrip("/").lower())
        if key in seen:
            continue
        seen.add(key)
    ...
    return {
        "statement": " ".join(clauses[-3:]) if clauses else thought.current_asu.ASU,
        "support_score": round(float(thought.score), 2),
        "raw_support_score": round(float(thought.score), 2),
        "confidence": "moderate",
        "match_target_a": 0.0,
        "match_target_b": 0.0,
        "lexical_target_a": 0.0,
        "lexical_target_b": 0.0,
        "source_diversity": {},
        "path_clauses": clauses[-6:],
        "supporting_clauses": [],
        "clause_records": records[-12:],
        "endpoint": thought.current_asu.ASU,
        "sources": list(thought.collected_sources),
        "notes": list(thought.collected_notes),
        "target_a": thought.start_target,
        "target_b": thought.target_b,
        "direction": getattr(thought, "route_id", ""),
        "route": (
            f"{getattr(thought, 'route_start_label', '')}->{getattr(thought, 'route_goal_label', '')}"
            if getattr(thought, "route_start_label", "") or getattr(thought, "route_goal_label", "")
            else ""
        ),
    }
...
def _load_cpp_thought_results(brain):
    if getattr(brain, "thought_results_loaded", False):
        return
    brain.thought_results_loaded = True
    output_path = getattr(brain, "thought_output_path", "")
    if not output_path or not os.path.exists(output_path):
        print("[THOUGHT] C++ thought engine produced no output.")
        return
        ...
        for raw_line in f:
            ...
            if parts[0] == "thought" and len(parts) >= 6:
                current = {
                    ...
                    "goal": int(parts[6]) if len(parts) >= 7 else -1,
                    "route": int(parts[7]) if len(parts) >= 8 else 0,
                    "steps": [],
                }
            elif parts[0] == "step" and current is not None and len(parts) >= 5:
                current["steps"].append(
                    {
                        "connection_index": int(parts[1]),
                        "from": int(parts[2]),
                        "to": int(parts[3]),
                        "reversed": int(parts[4]),
                    }
                )
    ...
    routes = list(getattr(brain, "target_thought_routes", []) or [])
    route_by_index = {int(route.get("index", 0) or 0): route for route in routes}
    if not route_by_index:
        completion_queries = completion_queries_for_side(brain, "b")
        route_by_index = {
            0: {
                "start_target": brain.current_target_a,
                "goal_target": brain.current_target_b,
                "goal_queries": completion_queries,
                "goal_agents": getattr(brain, "target_goal_agents", []),
                "seed_specs": getattr(brain, "target_seed_specs", []),
            }
        }
    seed_queries = {}
    ...
    offset_keys = _offset_key_map(brain)
    thoughts = []
    for record in records:
        ...
        thought = Thought(
            start_agent,
            seed_query=seed_queries.get((int(record.get("route", 0) or 0), record["start"]), ""),
            start_target=route.get("start_target", ""),
            target_b=route.get("goal_target", ""),
            success_queries=route.get("goal_queries", []),
            goal_agents=route.get("goal_agents", []),
        ...
        thought.alive = False
        thought.termination_reason = record["reason"]
        thought.successful = bool(record["success"] and len(thought.history) > 1)
        if thought.successful:
            try:
                thought.success_payload = thought._build_relationship_statement()
            except Exception:
                thought.success_payload = _fallback_success_payload(thought)
            if isinstance(thought.success_payload, dict):
                thought.success_payload["endpoint_reached"] = True
                thought.success_payload["direction"] = route.get("id", "")
                thought.success_payload["route"] = (
                    f"{route.get('start_label', '')}->{route.get('goal_label', '')}"
                )
        thoughts.append(thought)

    brain.thoughts = thoughts
    print(
        f"[THOUGHT] Loaded {len(thoughts)} C++ thought paths; "
        f"{sum(1 for thought in thoughts if thought.successful)} reached assigned endpoints."
    )
...
def start_thought_workers(brain, stop_event, worker_count=None):
    ...
    refresh_target_seed_agents(
        ...
        create_python_thoughts=False,
    )
    brain.thought_expected_count = _assigned_cpp_thought_count(brain)
    ...
    brain.thought_results_loaded = False
    brain.thought_process = None
    ...
    if not os.path.exists(binary_path):
        print("[THOUGHT] C++ thought engine is not built. Run build.sh to compile thought_engine.")
        brain.thought_results_loaded = True
        with brain._thought_lock:
            brain.thought_active_count = 0
        return

    unique = f"{os.getpid()}_{brain.thought_generation}"
    brain.thought_input_path = os.path.join(tempfile.gettempdir(), f"brain_thought_{unique}.in")
    brain.thought_output_path = os.path.join(tempfile.gettempdir(), f"brain_thought_{unique}.out")
    _write_cpp_thought_input(brain, brain.thought_input_path)

    env = os.environ.copy()
    env["SHM_POS"] = brain.shm_pos.name
    env["SHM_CONNECTIONS"] = brain.shm_connections.name
    env["THOUGHT_INPUT"] = brain.thought_input_path
    env["THOUGHT_OUTPUT"] = brain.thought_output_path
    brain.thought_process = subprocess.Popen([binary_path], env=env)
    print(
        f"[THOUGHT] Launched C++ thought engine for {brain.thought_expected_count} thought agents "
        f"({THOUGHT_AGENTS_PER_WORKER} argument variants per worker)."
    )
...
def thought_workers_finished(brain):
    ...
    if process is not None and process.returncode not in (0, None):
        print(f"[THOUGHT] C++ thought engine exited with code {process.returncode}.")
...
def bootstrap_thought_histories(brain, seed_limit=None):
    ...
    brain.thought_expected_count = _assigned_cpp_thought_count(brain)
    binary_path = _thought_binary_path()
    if not os.path.exists(binary_path) or brain.thought_expected_count <= 0:
        return completed_thought_histories(brain)

    unique = f"{os.getpid()}_{brain.thought_generation}_bootstrap"
    brain.thought_input_path = os.path.join(tempfile.gettempdir(), f"brain_thought_{unique}.in")
    brain.thought_output_path = os.path.join(tempfile.gettempdir(), f"brain_thought_{unique}.out")
    brain.thought_results_loaded = False
    _write_cpp_thought_input(brain, brain.thought_input_path)
    ...
    env["THOUGHT_INPUT"] = brain.thought_input_path
    env["THOUGHT_OUTPUT"] = brain.thought_output_path
    subprocess.run([binary_path], env=env, check=False, timeout=30)
    _load_cpp_thought_results(brain)
