...
def _connection_has_specific_information(graph, key):
...
def _expand_with_specific_adjacent_context(graph, adjacency, core_nodes, kept_nodes, kept_edges):
    specific_nodes = set(); specific_edges = set()
    for node in set(core_nodes):
        for neighbor, key in adjacency.get(int(node), ()):
            if not _connection_has_specific_information(graph, key): continue
...
def _anchor_candidate_limit(graph):
    candidate_multiplier = max(1, int(ACTIVE_SUBGRAPH_CANDIDATE_MULTIPLIER)); base_limit = max(TARGET_SEED_LIMIT, GOAL_AGENT_LIMIT); total_agents = max(len(getattr(graph, "agents_by_idx", {}) or {}), len(getattr(graph, "agents", {}) or {}))
...
def _select_functioning_anchors(graph, total_thoughts):
    thought_process.clear_thought_caches(); candidate_limit = _anchor_candidate_limit(graph); selected_routes = thought_process.select_thought_routes(graph, seed_limit=candidate_limit, goal_limit=candidate_limit, total_thoughts=total_thoughts); thought_process.apply_thought_routes(graph, selected_routes)
...
def _clear_agent_connectors(graph):
    for agent in graph.agents_by_idx.values():
        agent.connectors = []
# Zeros positions for agents removed from the active subgraph.
def _zero_inactive_agent_positions(graph, active_nodes):
    for idx in list(graph.agents_by_idx):
        if int(idx) in active_nodes: continue
        offset = int(idx) * AGENT_POSITION_RECORD_BYTES; struct.pack_into("ffffff", graph.shm_pos.buf, offset, 0, 0, 0, 0, 0, 0)
...
def _repack_connections(graph, kept_edges):
    old_offsets = dict(graph.connection_offsets); old_count = int(graph.connection_count); kept_keys = sorted(kept_edges, key=lambda key: old_offsets.get(key, 10**12)); _clear_agent_connectors(graph); graph.connection_offsets = {}; graph.connection_buckets = {}; graph.seen_connections = set()
    for new_index, key in enumerate(kept_keys):
        old_offset = old_offsets.get(key)
        if old_offset is None: continue
        subject_sp, predicate_sp = p_connection_graph._state_for_key(graph, key); utility = p_connection_graph._read_connection_utility(graph, old_offset); new_offset = 4 + (new_index * CONNECTION_RECORD_SIZE); s_idx, o_idx, relation_id = int(key[0]), int(key[1]), int(key[2])
        p_connection_graph._pack_connection_record(graph, new_offset, s_idx, relation_id, o_idx, utility, subject_sp, predicate_sp); p_connection_graph._register_connection_key(graph, key, offset=new_offset); specifics = graph.connection_specifics.get(key, {})
        connector = Connector(graph.shm_connections.buf, new_offset, graph.agents_by_idx, subject_sp=subject_sp, predicate_sp=predicate_sp, source=graph.connection_sources.get(key, "unknown"), evidence_text=graph.connection_texts.get(key, ""), subject_specifics=specifics.get("subject_specifics"), predicate_specifics=specifics.get("predicate_specifics"), connection_specifics=specifics.get("connection_specifics"), previous_agent_ids=graph.connection_previous_agents.get(key, ()),)
        p_connection_graph._attach_connector(connector)
    graph.connection_count = len(kept_keys); struct.pack_into("i", graph.shm_connections.buf, 0, graph.connection_count); clear_start = 4 + (graph.connection_count * CONNECTION_RECORD_SIZE); clear_end = 4 + (old_count * CONNECTION_RECORD_SIZE)
    if clear_end > clear_start:
        graph.shm_connections.buf[clear_start:clear_end] = b"\x00" * (clear_end - clear_start)
    graph.connection_sources = _filter_connection_dict(graph.connection_sources, kept_edges); graph.connection_states = _filter_connection_dict(graph.connection_states, kept_edges); graph.connection_specifics = _filter_connection_dict(graph.connection_specifics, kept_edges)
    graph.connection_previous_agents = _filter_connection_dict(graph.connection_previous_agents, kept_edges); graph.connection_texts = _filter_connection_dict(graph.connection_texts, kept_edges)
# Filters filter anchor state for active subgraph pruning.
def _filter_anchor_state(graph, active_nodes):
    filtered_routes = []
    for route in list(getattr(graph, "target_thought_routes", []) or []):
    ...
    thought_process.apply_thought_routes(graph, filtered_routes)
# Keeps only route-relevant nodes and connections before final thinking and synthesis.
def prepare_active_subgraph(graph, total_thoughts=None):
    total_thoughts = int(total_thoughts or (THINK_THREADS * THOUGHT_AGENTS_PER_WORKER)); all_keys = set(graph.connection_offsets); directed_adjacency = _connection_adjacency(all_keys, directed=True); context_adjacency = _connection_adjacency(all_keys, directed=False); _select_functioning_anchors(graph, total_thoughts); core_nodes, core_edges = set(), set()
    route_summaries = []
    for route in list(getattr(graph, "target_thought_routes", []) or []):
    ...
    kept_nodes = set(core_nodes); kept_edges = set(core_edges); specific_nodes, specific_edges = _expand_with_specific_adjacent_context(graph, context_adjacency, core_nodes, kept_nodes, kept_edges); kept_nodes, kept_edges = _expand_with_adjacent_context(context_adjacency, core_nodes, kept_nodes, kept_edges)
    context_node_count = max(0, len(kept_nodes) - len(core_nodes)); context_edge_count = max(0, len(kept_edges) - len(core_edges)); removed_connections = max(0, int(graph.connection_count) - len(kept_edges)); removed_nodes = max(0, len(graph.agents_by_idx) - len(kept_nodes)); _repack_connections(graph, kept_edges)
    _zero_inactive_agent_positions(graph, kept_nodes); _filter_anchor_state(graph, kept_nodes)
    summary = {"seed_count": sum(item["seed_count"] for item in route_summaries), "goal_count": sum(item["goal_count"] for item in route_summaries), "active_seed_count": len(graph.target_seed_agents), "active_goal_count": len(graph.target_goal_agents), "kept_nodes": len(kept_nodes), "removed_nodes": removed_nodes, "kept_connections": len(kept_edges), "removed_connections": removed_connections, "context_nodes": context_node_count, "context_connections": context_edge_count, "specific_context_nodes": len(specific_nodes), "specific_context_connections": len(specific_edges - core_edges), "routes": route_summaries, "anchor_pair_diagnostics": list(getattr(graph, "anchor_pair_diagnostics", []) or []),}
    route_text = "; ".join(f"{item['route']} seeds {item['seed_count']} goals {item['goal_count']} core {item['core_connections']}" for item in route_summaries) or "none"; diagnostics = list(getattr(graph, "anchor_pair_diagnostics", []) or [])
    for item in diagnostics:
        status = "selected" if item.get("selected") else "rejected"; graph._conn_log.append(
