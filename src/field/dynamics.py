...
# consecutive steps). Pure gradient flow on coercive E (barrier + anchor) ⇒ E monotone
# non-increasing ⇒ the gather always settles. Returns (X_hist [T,N,d], E_hist [T]).
def rollout(X0: Tensor, C: Coupling, cfg: FieldConfig, anchor: Anchor | None = None) -> tuple[Tensor, Tensor]:
    X = X0.clone()
    X_hist, E_hist = [X.clone()], [compute_E(X, C, cfg, anchor).item()]
    settled = 0
    for _ in range(cfg.H_max):
        X_prev = X
        X = step(X, C, cfg, anchor)
        X_hist.append(X.clone()); E_hist.append(compute_E(X, C, cfg, anchor).item())
