...
# Runs the Brain loop, ingesting extracted connections and responding to dashboard commands.
def run_brain(scrape_queue, asu_queue, stop_event, shm_names, sync_counter, ingest_counter):
    global _ACTIVE_BRAIN; print("[BRAIN] Brain Online. Awaiting input..."); brain = Brain(shm_names); _ACTIVE_BRAIN = brain
    while not stop_event.is_set():
        ...
        if cmd_raw:
            ...
            if cmd_id != brain.last_cmd_id and current_status == PHASE_IDLE:
                ...
                brain.reset_research_state(); brain.finalized_command_ids.discard(cmd_id); brain.clear_report(); brain.shm_status.buf[0] = PHASE_RESEARCH; print(f"[BRAIN] New command: '{cmd_raw}'"); print(f"[BRAIN] Target A: '{brain.current_target_a}' | Target B: '{brain.current_target_b}'"); _set_counter(sync_counter, 1)
                try:
                    ...
                    if queries:
                        ...
                        print("[BRAIN] No sub-queries generated. Returning to idle."); brain.shm_status.buf[0] = PHASE_IDLE; _set_counters(sync_counter, ingest_counter, 0)
                except Exception as e:
                    print(f"[BRAIN] Query expansion failed: {e}"); brain.shm_status.buf[0] = PHASE_IDLE; _set_counters(sync_counter, ingest_counter, 0)
        while True:
        ...
        time.sleep(0.1)
# Starts native thought routing for the current Brain if it is ready.
def launch_thought_workers(stop_event, worker_count=THINK_THREADS):
    ...
    return True
# Prunes the Brain graph to the route-supporting subgraph after physics settles.
...
# Stops any active thought process owned by the Brain.
def stop_thought_workers():
    brain = _ACTIVE_BRAIN
    if brain is None: return
    brain.stop_thought_workers()
# Queues the secondary scrape pass from Brain refinement queries.
