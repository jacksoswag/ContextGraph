...
MECHANISM_DIVERSITY_THRESHOLD = 0.72
MECHANISM_TOKEN_RE = re.compile(r"[a-z0-9]+")
...
def _selected_thought_path_concrete_count(self, thought):
    ...
    for step in list(getattr(thought, "history", []) or [])[1:]:
        if not isinstance(step, dict):
            continue
        fields = (
            step.get("subject_specifics", []),
            step.get("predicate_specifics", []),
            step.get("connection_specifics", []),
        )
...
def _thought_rank_key(self, thought):
    return (
        _selected_thought_path_concrete_count(self, thought),
        _selected_thought_concrete_count(self, thought),
        _selected_thought_source_count(self, thought),
        float((getattr(thought, "success_payload", {}) or {}).get("support_score", 0.0)),
        _thought_chain_char_count(self, thought),
...
def _thought_target_tokens(self):
    return set(
        target_tokens(
            f"{getattr(self, 'current_target_a', '')} {getattr(self, 'current_target_b', '')}",
            min_length=3,
        )
    )
...
def _thought_mechanism_tokens(self, thought):
    target_token_set = _thought_target_tokens(self)
    texts = []
    history = list(getattr(thought, "history", []) or [])
    if history and isinstance(history[0], dict):
        for step in history[1:]:
            ...
            try:
                ...
                relation_id = -1
            relation = literal_from_index(relation_id) or ""
            texts.extend(
                [
                    step.get("subject", ""),
                    relation,
    ...
    if isinstance(payload, dict):
        texts.extend(
            ...
            for record in self.synthesizer._payload_role_records(payload, "path")
        )

    tokens = set(target_tokens(" ".join(str(text or "") for text in texts), min_length=3))
    return {
        token
        for token in tokens
        if token not in target_token_set
        and not token.isdigit()
        and len(MECHANISM_TOKEN_RE.findall(token)) > 0
    }
...
def _thought_payload_signature(self, thought):
    payload = getattr(thought, "success_payload", None)
    if isinstance(payload, dict):
        records = self.synthesizer._payload_role_records(payload, "path")
        text = " ".join(
    ...
    return tuple(
        ...
        for step in getattr(thought, "history", []) or []
    )
...
def _select_successful_thoughts(self, thoughts):
    ...
    ranked = sorted(endpoint_thoughts, key=lambda thought: _thought_rank_key(self, thought), reverse=True)
    selected = []
    selected_mechanisms = []
    seen = set()
    limit = max(1, int(SYNTHESIS_ARGUMENT_LIMIT))
    ...
    def try_append(thought, enforce_diversity):
        ...
        if enforce_diversity and selected_mechanisms and mechanism_tokens:
            similarity = max(
                _mechanism_similarity(mechanism_tokens, selected_tokens)
...
def run_final_synthesis(self):
    ...
    if successful_thoughts:
        selected_successful = _select_successful_thoughts(self, successful_thoughts)
        ...
        selected_chars = sum(_thought_chain_char_count(self, thought) for thought in selected_successful)
        print(
            f"[SYNTHESIS] Running for '{self.current_target}' with "
            f"the merged top {len(selected_successful)} of {len(successful_thoughts)} endpoint chains "
            f"({selected_chars} distinct path characters)..."
        )
